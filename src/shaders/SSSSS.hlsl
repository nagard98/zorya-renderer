...
#define GOLUBEV_MODEL_ID 3
#define JIMENEZ_SEPARABLE_MODEL_ID 2
#define JIMENEZ_GAUSS_MODEL_ID 1
...
cbuffer draw_constants : register(b1)
{
    float2 dir;
    float sd_gauss_jim;
    float pad2;
...
{
    return (exp(-r / d) + exp(-r / (3.0f * d))) / (8.0f * PI * d); //rR(r)
}
...
PS_OUT ps(float4 fragPos : SV_Position)
{
    ...
    switch(pixel_sss_model_id)
    {
        case GOLUBEV_MODEL_ID:
            {
        ...
        case JIMENEZ_SEPARABLE_MODEL_ID:
            {            
                ...
                float scale = distanceToProjectionWindow / linearMiddleDepth;
                float sssWidth = currentSSSModel.meanFreePathDist;
            
                float rot_threshold = sssWidth * currentSSSModel.scale;
	
	        // Calculate the final step to fetch the surrounding pixels:
                float finalStep = sssWidth * (1.0f / floor(numSamplesJimenezSep/2.0f)) * scale /** dir*/;
            //finalStep *= 1.0f; //SSSS_STREGTH_SOURCE; // Modulate it using the alpha channel.
                /// (2.0 * meanFreePathDist); // sssWidth in mm / world space unit, divided by 2 as uv coords are from [0 1]
            ...
            // Accumulate the center sample:
                ...
                else
                {
                    ...
                    float sinRandAngle = sin(randAngleJitt);
                    new_axis = float2(cosRandAngle, sinRandAngle);
                    spec = specularIrrad.SampleLevel(texSampler, uvCoord, 0).rgb;
                }
        ...
        case JIMENEZ_GAUSS_MODEL_ID:
            {
                ...
                if (dir.x == 1)
                {
                    float ddx_depth = ddx(linDepth);
                    scale_factor = sssLevel / (linDepth + correction * min(abs(ddx_depth), maxdd));
                }
                else
                {
                    float ddy_depth = ddy(linDepth);
                    scale_factor = sssLevel / (linDepth + correction * min(abs(ddy_depth), maxdd));
                }
